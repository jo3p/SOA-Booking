...
class QueryDisplay(Resource):
    def get(self):
        ...
        result = MergeQueries.merge(query, accomodations, prices, review_scores).to_dict(orient='records')
        return result, 200
...
class MergeQueries:
    def merge(query_results, accomodations, prices, review_scores):
        ...
        ranked_db = pd.DataFrame(list(zip(accomodations, prices, review_scores)),
                                 columns=['accomodations', 'prices', 'review_scores'])
        ...
        ranked_db = ranked_db[['name', 'city', 'country', 'prices', 'review_scores']]  # Rearange column order
        ranked_db.columns = ['Name', 'City', 'Country', 'Price p.p', 'Review Score']  # Rename columns
        return ranked_db